...
has_bonked: .byte 0
is_falling: .byte 0
has_stopped_falling: .byte 0

# Other flags
requesting_puzzle: .byte 0
...
main:
    ...
    or      $t4, $t4, FALLING_INT_MASK   # enable puzzle interrupt
    or      $t4, $t4, STOP_FALLING_INT_MASK   # enable puzzle interrupt
...
loop: # Use this loop to perform flag-checks repeatedly

    check_out_of_water:
    # Exit if we already are awaiting a puzzle
    lb      $t0, requesting_puzzle
    bne     $t0, 0, end_out_of_water
    # Exit if water level is over 1,000,000 units
    lw      $t0, GET_WATER_LEVEL
    bgt     $t0, 1000000, end_out_of_water
    
    la      $t1, board
    sw      $t1, REQUEST_PUZZLE

    li      $t0, 1
    sb      $t0, requesting_puzzle
    end_out_of_water:

    check_has_puzzle:
    # Check if flag is raised (1)
    lb      $t0, has_puzzle
    beq     $t0, 0, end_has_puzzle
    ...
    # Activate the powerwash
    li      $a1, 4  # Radius = 4
    li      $a2, 0  # X-offset = 0
    li      $a3, -2  # Y-offset = -2 (2 upwards)
    jal     form_word
    sw      $v0, POWERWASH_ON
    ...
    # Check if flag is raised (1)
    lb      $t0, has_timer
    beq     $t0, 0, end_has_timer

    # Get the timer to go off 200000 cycles from now
    lw      $t0, TIMER
    add     $t0, $t0, 7500
    sw      $t0, TIMER

    # Attempt to go upwards (the falling interrupts will handle the case of jumping)
    li      $t0, 270
    sw      $t0, ANGLE
    li      $t0, 1
    sw      $t0, ANGLE_CONTROL
    ...
    check_has_stopped_falling:
    # Check if flag is raised (1)
    lb      $t0, has_stopped_falling
    beq     $t0, 0, end_has_stopped_falling

    # Arithmetic hack to do the following:
    # If on left of map, turn right
    # If on right of map, turn left
    lw      $t0, BOT_X
    slti    $t0, $t0, 19    # t0 = BOT_X < 19? 1 : 0
    mul     $t0, $t0, -180   # t0 *= -180 
    add     $t0, $t0, 180   # t0 += 180; Left-side = 0 degrees, right-side = 180 degrees
    sw      $t0, ANGLE
    li      $t0, 1
    sw      $t0, ANGLE_CONTROL
    ...
    # Reset the flag to 0
    li      $t0, 0
    sb      $t0, has_stopped_falling
    end_has_stopped_falling:
